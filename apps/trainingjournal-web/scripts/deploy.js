...
const thisWorkspace = '@tbergq/trainingjournal';
...
(async () => {
  try {
    ...
    fs.copyFileSync(
      ...
      path.join(buildDir, 'apps', 'trainingjournal-web', '.babelrc.js'),
    );
    fs.copyFileSync(
    ...
    const packageJson = require(path.join(buildDir, 'apps', 'trainingjournal-web', 'package.json'));
    packageJson.engines = { node: '10.x' };
    fs.writeFileSync(
      path.join(buildDir, 'apps', 'trainingjournal-web', 'package.json'),
